...
- **Grafico obrigatorio ausente segura so Discord e WhatsApp.** Com `SETUP_NOTIFY_REQUIRE_CHART_FOR_ENTRY` ligado (padrao) e o grafico falhando, o `setup-live` calava todos os canais, inclusive o Telegram, que nunca recebe imagem; agora so os canais com imagem deixam de receber, e o log diz isso. O watcher manda o texto mesmo sem grafico, como sempre fez. O WhatsApp do scanner, que saia so com texto quando o grafico faltava, passa a ser segurado como o Discord.
...
- **`doctor` avisa API key de CEX que pode sacar** (ADR 0007, ROADMAP 1.7). Binance, Bybit e OKX expoem a permissao da propria key; o check `cex_key_sem_saque` pergunta e avisa se ela sacar. Nas outras CEXs, e quando a consulta falha, o check diz "nao verificado": a duvida nunca e tratada como "sem saque". So consulta com credencial configurada.
- **`doctor` avisa quando o OpenClaw executa sem pedir aprovacao** (`openclaw_aprovacao`), lendo `openclaw exec-policy show --json`: `security=full` (ou `ask=off`) e real autonomo; o modo `auto` passa pelo revisor automatico antes do operador. E avisa `AUTO_WITHDRAW_ENABLED=true`, o saque automatico do `auto_trade_nado.py` (`saque_automatico`).
- **Bloqueio configuravel.** Por decisao do autor, saque e falta de aprovacao sao aviso. `BLOQUEAR_SAQUE=sim` e `BLOQUEAR_SEM_APROVACAO=sim` transformam o aviso em bloqueio: o `doctor` reprova e o comando de trade e recusado. Nao verificado nunca bloqueia. Valor fora do vocabulario de booleano reprova o `doctor` nomeando a variavel.
...
- **O `setup-live` entrega so pelos canais do OpenClaw do operador.** O Discord ia direto pela API REST, com um token de bot que a skill lia do ambiente e ate do `~/.openclaw/openclaw.json`, e o fallback para o OpenClaw vinha desligado -- uma falha descartava a mensagem. Agora todo canal sai por `openclaw message send`: **quem entrega no Discord precisa te-lo configurado como canal no OpenClaw antes de atualizar**. Bot dedicado passa a ser uma conta dedicada no OpenClaw (`SETUP_NOTIFY_ENTRY_ACCOUNT`); a conta `default` deixa de ser imposta. **Nao ha canal padrao:** `SETUP_NOTIFY_ENTRY_CHANNEL` vazio (era `telegram`) nao envia, e um destino sem canal gera aviso no log. As mencoes no Discord passam a seguir o OpenClaw. O scanner e o watcher ainda usam o caminho direto e migram na proxima PR.
...
- Topico do Telegram documentado e com flag: `SETUP_NOTIFY_ENTRY_THREAD_ID` / `--notify-entry-thread-id` viram `--thread-id` no `openclaw message send`.
