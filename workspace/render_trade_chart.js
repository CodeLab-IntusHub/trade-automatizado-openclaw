...
// Marca do badge: a mesma SETUP_NOTIFY_BRAND do texto do sinal. Vazia, sem badge.
function brandBadge() {
  const brand = String(process.env.SETUP_NOTIFY_BRAND || '').trim();
  return brand ? `<div class="badge">${escapeHtml(brand)}</div>` : '';
}
